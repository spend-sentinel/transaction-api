...
const postToDataBase = async (data: MoneyTransaction): Promise<number> => {
  ...
  return (
    await axios.post(url, JSON.stringify(data, null, 2), {
...
const updateTransactionsInDataBase = async (txns: Transaction[]) => {
  ...
  txns.forEach((bankTransaction) => {
    ...
    if (undefined !== identifier) {
      if (
        undefined ==
        transactionsInDataBase.find(
          (transaction: MoneyTransaction) => transaction.TransNum == identifier,
        )
      ) {
...
const updateDataBase = async () => {
  try {
    ...
    const scrapeResult: ScraperScrapingResult =
      await scraper.scrape(credentials);
